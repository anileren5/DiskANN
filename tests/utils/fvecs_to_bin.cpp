...
void block_convert(std::ifstream& reader, std::ofstream& writer,
  ...
  // Writes only the float data (without the dimension headers) to the output
  // file.
...
int main(int argc, char** argv) {
  ...
  writer.write((char*) &npts_s32,
               sizeof(_s32));  // The first 4 bytes are the number of points
  writer.write((char*) &ndims_s32,
               sizeof(_s32));  // The next 4 bytes are the number of dimensions
